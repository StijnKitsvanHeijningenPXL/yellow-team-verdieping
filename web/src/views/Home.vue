...
export default {
  ...
  let options = {
    ...
    body: JSON.stringify({
      client_id: 'IKkryIcdzIYlTQF92n5u6Y2ZsskRq7RN',
      client_secret: 'UxSUl3Oz1avydgo1e-zJtCZFQ5dtuOfKZ89J3CpfQt-UyGUdmBdrerqrlQcoPOHO',
      audience: 'https://SecKrcGenkAbbonees',
      grant_type: 'client_credentials'
    })
  };

  try {
    const response = await fetch('https://dev-qeikcy1n4qn7rptk.us.auth0.com/oauth/token', options);
    ...
    const resp = await fetch("http://localhost:5000/api/seatholders" , options);
